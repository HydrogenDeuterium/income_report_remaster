...
def get_months(s: str) -> Iterable[int]:
    """
    ...
    if s.startswith(('yr', 'YR', 'yR', 'Yr')):
        ...
        quarter = int(s[1])
        months = range(quarter * 3 - 2, quarter * 3 + 1)
    else:
        months = [int(s)]
    return months
...
def get_month_days(yearmonth=None, test=False) -> (int, list[MonthOutHome]):
    ...
    yearmonth = re.search(r"(\d{4})(\d{2}|[qQ][1-4])", text)
...
def get_structure(last_year: int, last_month: int, toml='./budget.toml', test=False) -> list[Category]:
    ...
    with open(toml, 'rb') as f:
        items_budgets: dict[str, dict[str, dict]] = tomllib.load(f)
